...
public static class PortDiagnostics
{
        ...
        bool companionActive, IReadOnlyList<DeviceView> devices)
    {
        ...
        var gameChecks = snapshot.Settings.Profiles.Select(profile =>
        {
            ...
            var definitions = driver.Ports(profile);
            ...
            var missing = definitions.Where(port => !(port.Protocol.Equals("TCP", StringComparison.OrdinalIgnoreCase) ? tcp : udp)
                .Contains(port.Port)).ToList();
            if (missing.Count == 0)
                return new GamePortCheck(profile.Id, profile.Name, ports, protocol, "Open on PC",
                    $"Windows sees {protocol} {string.Join(", ", ports)} listening locally. Public forwarding still needs a Friend join test.");
            if (run.State is "Starting" or "Process running")
                return new GamePortCheck(profile.Id, profile.Name, ports, protocol, "Opening",
                    "The process is starting; waiting for " + string.Join(", ", missing.Select(port => $"{port.Protocol} {port.Port}")) + ".");
            return new GamePortCheck(profile.Id, profile.Name, ports, protocol, "Closed on PC",
                ...
                string.Join(", ", missing.Select(port => $"{port.Protocol} {port.Port}")) + " listening.");
        }).ToList();

        var controlPort = snapshot.Settings.CompanionPort;
        var controlOpen = companionActive && tcp?.Contains(controlPort) == true;
