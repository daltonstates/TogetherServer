...
3. Saved servers appear in a grid on desktop and stack on a phone. Each card shows **Start server** or **Stop server**, **Invite friend**, four connection checks, and **Settings**. **Add new server** opens the same setup dialog; **Cancel** discards unsaved changes, including when you cancel the first server. **Settings and safety** opens a separate dialog. Additional paths and health actions remain under **More server options**.
