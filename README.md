...
Choose **Minecraft Java Edition** or **Minecraft Bedrock Edition** in **My server** setup. Prepare the dedicated server folder yourself outside the source checkout, including `server.properties` with `level-name` and `server-port`. Enter those exact values as the world name and game port in TogetherServer. The app reads the files and starts only the saved local program; it does not download a server, accept terms, or rewrite configuration or worlds.

- **Java:** Choose an installed `java.exe` and the server JAR in the prepared folder. Review Minecraft's terms yourself and set `eula=true` in that folder before Start. [Official Java server download](https://www.minecraft.net/en-us/download/server).
- **Bedrock:** Choose `bedrock_server.exe` in its prepared folder. TogetherServer reads `server-portv6` and `enable-lan-visibility` too, including the default discovery ports when LAN visibility is on. [Official Bedrock server setup](https://learn.microsoft.com/en-us/minecraft/creator/documents/bedrockserver/getting-started?view=minecraft-bedrock-stable) and [properties](https://learn.microsoft.com/en-us/minecraft/creator/documents/bedrockserver/server-properties?view=minecraft-bedrock-stable).
