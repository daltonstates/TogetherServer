...
Every Ready built-in game server shows the latest online-player count returned by its local game protocol on both the Host and Friend cards. Valheim uses its Steam-compatible query reply on the configured port plus one; Java and Bedrock use the same local status replies that establish Ready. A missing or malformed reply is **Player count unavailable**, never zero.
...
Automatic shutdown is off by default. The owner can enable it and choose a 1-to-1440-minute wait under **Friend access → Stop & timer**. When a Ready server reports exactly zero players, the Host and every assigned Friend see the same live **Stops in** countdown. A positive or unavailable count, a running or unconfigured Host game check, or a missing, stale, unknown, or running game report from an assigned Friend PC cancels it; the cards explain that the timer is paused. At expiry, the Host repeats those checks and queries the count again inside the lifecycle gate before using only the game's existing graceful Stop path. It never force-kills a timed-out server. Reopening TogetherServer starts a fresh countdown because the app cannot prove the server stayed empty while it was closed.
