...
In Host mode, click **Find Valheim installs and saves**. The read-only search checks ready fixed and removable drives for common Steam folders (including `G:\Steam`), root-level custom folders containing `steamapps`, libraries listed in Steam's `libraryfolders.vdf`, and local world folders directly under a drive root. It also checks the current Windows user's local Valheim saves. It does not crawl every nested folder on every drive. Use **Browse for valheim_server.exe** to open a Windows file picker and choose an installation anywhere, including a deeper custom Steam path. If the tool is missing, **Install Valheim Dedicated Server in Steam** opens Steam's install flow only when you click it; confirm any install and terms yourself, then rescan. TogetherServer does not download a game binary or accept terms in the background. The [official server guide](https://www.valheimgame.com/support/a-guide-to-dedicated-servers/) also explains Steam's Tools install path.

To start with a single-player or locally hosted multiplayer save, close that local Valheim session and choose **Copy an existing save**. Click **Browse and copy a world save** and choose its `.db` or `.fwl` file inside a `worlds_local` folder on any drive; a matching pair is required. You can also pick a discovered world or enter its save root and exact filename. TogetherServer copies the pair into a separate folder under `%LOCALAPPDATA%\TogetherServer\world-imports` and points this profile at the copy. The source files are never moved or overwritten by the app. Steam's installation folder and Valheim's save folder can be different; the [official server guide](https://www.valheimgame.com/support/a-guide-to-dedicated-servers/) documents the default save path and `-savedir` override. If the world is in Steam Cloud or a legacy `worlds` folder, use Valheim's **Manage Saves → Move to Local** first; the [developer's save FAQ](https://steamcommunity.com/app/892970/discussions/0/591774445091193259/) explains the local/cloud distinction. You can select **Create a new seed** explicitly and supply an existing save root; Start refuses a new-world name when save files already exist there. No real world copy has yet been tested by this project.

Enter a server name and UDP start port, save the profile, then set its server password in the separate protected field. TogetherServer does not modify Steam's startup script or delete a world. The password is encrypted in the current Windows user's local data; Valheim still requires it in its launch arguments, which Windows users with sufficient process access may inspect.
