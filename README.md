...
In Host mode, click **Find Valheim installs and saves**. The read-only search checks common Steam folders on fixed drives, libraries listed in Steam's `libraryfolders.vdf`, and the current Windows user's local Valheim worlds. You can also enter an installed `valheim_server.exe` path yourself. If the tool is missing, **Install Valheim Dedicated Server in Steam** opens Steam's install flow only when you click it; confirm any install and terms yourself, then rescan. TogetherServer does not download a game binary or accept terms in the background. The [official server guide](https://www.valheimgame.com/support/a-guide-to-dedicated-servers/) also explains Steam's Tools install path.

To start with a single-player or locally hosted multiplayer save, close that local Valheim session and choose **Copy an existing save**. Pick a discovered local world or enter the save root and exact world filename. TogetherServer requires the matching `worlds_local/<name>.db` and `.fwl` files, copies them into a separate folder under `%LOCALAPPDATA%\TogetherServer\world-imports`, and points this profile at the copy. The source files are never moved or overwritten by the app. If the world is in Steam Cloud, use Valheim's **Manage Saves → Move to Local** first, then rescan; the [developer's save FAQ](https://steamcommunity.com/app/892970/discussions/0/591774445091193259/) explains the local/cloud distinction. You can select **Create a new seed** explicitly and supply an existing save root; Start refuses a new-world name when save files already exist there. No real world copy has yet been tested by this project.
