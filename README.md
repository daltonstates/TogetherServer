...
For a player on the Host PC, Valheim's **Join IP** can use `127.0.0.1:<game port>`; this address cannot reach the Host from a Friend PC. In Host **Friends → Valheim game join**, enter and save your public IPv4 address. The app then shows and copies `<public IP>:<game port>` for each Valheim server in both **Servers** and **Friends**. A paired Friend also sees that address beside the server in Friend mode. This is an owner-entered candidate address, not a tested public connection. The Steam backend needs the game UDP port and following port reachable; the [official server guide](https://www.valheimgame.com/support/a-guide-to-dedicated-servers/) explains this and says Crossplay can also join by a game-generated code. TogetherServer does not read or create Valheim's game join code.
