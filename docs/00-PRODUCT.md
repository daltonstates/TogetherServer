...
- **Owner playing locally:** the Host sees the same server-reported count as Friends. No local game-process path or indicator is used for automatic or remote Stop.
...
- Server-reported player counts are the only occupancy signal. Valheim prefers its local query; a non-Crossplay no-reply case may use only a complete connection history from TogetherServer's own run log. A malformed query or incomplete log sequence is Unknown, never zero. Host and Friend game-running checks and paths do not exist.
- When enabled, each Ready zero-player server owns a Host-generated shutdown deadline shown as the same live countdown on its Host card and every assigned Friend card. A positive or Unknown count cancels that deadline. The Host may add an entered number of minutes to the active deadline; that extension applies only to the current countdown. When no deadline exists, both cards show the count-based timer reason. A Host app restart starts a fresh idle window rather than counting unobserved time.
