...
- **Friend mode:** each friend runs the same app on the PC used to play. Each saved server invite has a distinct credential. The app sends authenticated heartbeats, displays Host/server reachability and the server-reported online-player count, and requests only actions the owner grants.
...
5. A permitted friend may request Stop only when the Ready game server returns a fresh online-player count of zero. Host repeats that query immediately before the graceful stop signal. A positive, missing, malformed, or unknown count denies remote Stop. The owner can request local Stop independently, including while players are online.
...
- Configure idle minutes, game executable path, world/profile, server name, port, remote-control toggle, and per-friend Start/Stop permissions. No player IDs are required. Save secret material through Windows-protected storage, outside the repo.
