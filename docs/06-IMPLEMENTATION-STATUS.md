...
| Check | Result |
| --- | --- |
| Temporary candidate build | Pass: npm reported 0 vulnerabilities; TypeScript/Vite, three fixtures, and a self-contained Windows x64 EXE built. The existing WebView2/WindowsBase `MSB3277` warning remains. It was not left as the installed release after the owner clarified that no release was requested. |
...
The mistakenly published `v0.1.5` GitHub release and tag were deleted immediately after the owner's correction. The local install was restored to v0.1.4. Future GitHub releases require a new explicit owner request.
