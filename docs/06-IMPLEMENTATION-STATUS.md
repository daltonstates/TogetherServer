...
## 2026-09-22 - Explicit per-Friend server assignments

Starting Git HEAD: `695e7eea`, clean `main` worktree. A `TS3` code now seeds a device credential with exactly the server profile encoded by that code. Host pairing data separately records the code that issued a credential and the current explicit set of server assignments. Authenticated status returns only assigned profiles, and every Start/Stop request rechecks the requested profile against that set before entering lifecycle code. The loopback-only **Friend access** screen lets the Host assign a paired PC to zero, one, or several saved servers; the existing Start and Stop permissions apply across those assignments. Unknown and duplicate profile IDs fail closed.

Existing scoped credentials migrate to their original profile. Older unscoped `TS1`/`TS2` credentials migrate to no profiles because their data cannot prove which server was intended; the Host may assign them after pairing. Refreshing a server code still revokes credentials issued by that code, including extra assignments on those credentials, without revoking credentials issued by a different server code. Removing a saved profile prunes it from assignment lists, while deleting the profile that issued a credential preserves the existing fail-closed revocation behavior.

| Final check | Result |
| --- | --- |
| `scripts/build.ps1 -ReleaseName release-candidate` | Pass: npm reported 0 vulnerabilities; TypeScript/Vite, three fixtures, and a self-contained Windows x64 EXE built. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| `TogetherServer.CompanionChecks` | Pass: 21 groups, 0 failures against the final candidate. Added coverage proves legacy migration, one-server code scope, wrong-server denial, unknown/duplicate-ID rejection, zero/one/multiple Host reassignment, immediate removal, restart persistence, scoped code rotation, and the existing remote action safety gates. Disposable data: `local-data/companion-checks/19691c3e95bc4187bec407bb4c81bd10`. |
| `TogetherServer.Checks` | Pass: 11 groups, 0 failures after updating the diagnostics fixture to declare its assigned profile. Disposable data: `local-data/checks/5c0f6d0a9b214900a2c3592a5a86bb7c`. |
| Served and native desktop smokes | Pass: 20 served and 19 native desktop groups, 0 failures. The candidate contains the new assignment labels; its React UI rendered in a visible WebView and accepted a compact 390x600 logical-pixel window. Disposable data: `local-data/served-smoke/7c44abcd17a2422b9614e3a972e046e8` and `local-data/desktop-smoke/8ea82eda73e44d01890d7ef7eee35887`. |
| Packaged candidate | `local-data/release-candidate/TogetherServer.exe`, file version `0.1.4.0`, 64,192,974 bytes, SHA-256 `63CF3E4B00AD30D2DF05148C362C73B03392BE1E398801B1946288692D0B83BC`. It was not installed, published, or pushed. |
| Browser and external acceptance | The Browser runtime listed no available browser, so manual browser screenshots and assignment interactions were not run. The native WebView smoke passed, but it does not replace that inspection. No real game, valued world, public Friend network, router, firewall, or DNS setting was used or changed. |

## 2026-09-22 - Scalable Friend server picker

Starting Git HEAD: `77fa643`, clean `main` worktree. The Friend PC card no longer repeats every saved server as an inline checkbox. It keeps a fixed-size server-access summary with the assignment count and a short name preview. **Choose servers** opens a separate picker with search, a bounded scrolling list, selected/total count, Select all, Clear all, Cancel, and one Save access action. Start and Stop permissions remain visible as compact controls on the PC card. This keeps each paired-PC card compact as the Host's server library grows.

| Final check | Result |
| --- | --- |
| React/TypeScript and Windows publish | Pass: npm reported 0 vulnerabilities; TypeScript/Vite, three fixtures, and the Windows x64 single-file publish completed. The scripted final copy to `local-data/release-candidate` was blocked because that earlier candidate was still running, so it was left untouched and the completed publish output was copied to a separate test path. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Large-library assignment | Pass with isolated data: 32 saved servers and existing disposable Friend devices loaded together; one device accepted all 32 assignments in one loopback-only update and reported all 32 afterward. Disposable data: `local-data/friend-access-ui-test/c360ffda39c647ed90921d0bc07a19c7`. |
| Served and native desktop smokes | Pass: 20 served and 19 native desktop groups, 0 failures. The served suite requires the searchable picker controls and bounded-list CSS, rejects the former unbounded inline checklist, and exercised the exact packaged EXE. Disposable data: `local-data/served-smoke/06c9988779a943c6ae20adff0ad31be2` and `local-data/desktop-smoke/f9284d328afe4a2cb3b4e8ca4f991eb7`. |
| Packaged candidate | `local-data/friend-access-candidate/TogetherServer.exe`, file version `0.1.4.0`, 64,194,199 bytes, SHA-256 `5F56CA3B68E3B28EFD1BCAB072543D9FA6866BC7314F5E79C95CF41BD29743E8`. It was not installed, published, or pushed. |
| Browser and external acceptance | The Browser runtime listed no available browser, so screenshot-level inspection of the card and picker was not run. The visible native WebView smoke and compact-window check passed, but they do not replace a manual visual review. No real game, world, credential, public listener, router, firewall, or DNS setting was used or changed. |

## 2026-09-22 - Shared player count and empty-server countdown

Starting Git HEAD: `70876ca2`, clean `main` worktree. Host and assigned Friend server cards now share one player-count presentation and the same Host-owned shutdown deadline. Automatic shutdown remains off by default. The owner can enable it and choose a 1-to-1440-minute wait under **Stop & timer**. A Ready server starts a countdown only after its built-in driver reports exactly zero online players. A positive or unavailable count cancels the deadline instead of pausing it.

The timer also fails closed around game activity. The Host game-client check must be configured and report closed. Every paired, non-revoked Friend PC assigned to that server must have a fresh heartbeat and report its game closed; a missing, stale, unknown, or running report cancels the deadline and gives Host and Friend cards a plain-language reason. Changing the configured wait starts a full new window. At expiry, the Host repeats the player and activity checks inside the serialized lifecycle gate and uses only the existing graceful game-driver Stop path. A failed Stop is never escalated to a force kill. Restarting TogetherServer starts a new observation window because it cannot prove the server stayed empty while the app was closed.

| Final check | Result |
| --- | --- |
| React/TypeScript build | Pass: 21 modules bundled, 0 errors. |
| `TogetherServer.ValheimChecks` | Pass: 9 groups, 0 failures. Added virtual-time coverage for zero-player deadlines, Friend/Host activity blockers, wait changes, positive and Unknown resets, no early stop, final zero recheck, and graceful Ctrl+C. Disposable data: `local-data/valheim-checks/e5dce0aea3054aa2a96e015e09b7d4fb`. |
| `TogetherServer.Checks`, `TogetherServer.MinecraftChecks`, and `TogetherServer.MinecraftSetupChecks` | Pass: 11, 3, and 4 groups, 0 failures. Java and Bedrock fixture counts both publish or cancel deadlines through the shared driver seam. |
| `TogetherServer.CompanionChecks` against the packaged candidate | Pass: 21 groups, 0 failures. The paired Friend received the same 0/10 count and exact deadline as the Host; a one-player report removed the deadline; the existing zero-only HTTPS Stop still used graceful Ctrl+C. Disposable data: `local-data/companion-checks/693cdb8d7cc24378b1c6ab09ca56f984`. |
| Served and native desktop smokes against the candidate | Pass: 20 served and 19 native desktop groups, 0 failures. The packaged UI contains the timer controls, count/countdown styles, and deadline API; the native WebView rendered and completed its lifecycle, picker, tray, and startup checks. Disposable data: `local-data/served-smoke/58b8cf52ac3542b0bb823dcd22c05951` and `local-data/desktop-smoke/38f8344fb1e14b63bfc13cd04c831a98`. |
| Packaged candidate | `local-data/release-candidate/TogetherServer.exe`, file version `0.1.4.0`, 64,198,557 bytes, SHA-256 `D7DCE96E7245B95920363D9801657C820411C6F4ADFBE75B6BC4B3CA0E5866D1`. The existing WebView2/WindowsBase `MSB3277` warning remains. The candidate was not installed, published, or pushed. |
| Browser and real-game acceptance | The in-app Browser listed no available browser, so screenshot-level inspection was not run. Native WebView rendering and served asset/API checks passed but do not replace it. Real Valheim/Minecraft player transitions, client joins, save/restart behavior after automatic Stop, and an outside Friend network were not run; fixture evidence is not a real-game claim. |

No real game binary, world, credential, public listener, router, firewall, or DNS setting was used or changed.

## 2026-09-22 - Private Valheim player count recovery and timer explanations

Starting Git HEAD: `56a90b4`, clean `main` worktree. A running private non-Crossplay Valheim server was Ready but did not answer its local Steam-compatible player query, so its count stayed Unknown and no automatic-shutdown deadline could exist. The Valheim driver still prefers that query. Only a true no-reply case may now use the exact TogetherServer-owned run log: `Game server connected` establishes the initial zero, complete connection/close event pairs update the count, and Valheim's periodic `Connections` checkpoint replaces it. A partial, contradictory, malformed, or over-capacity log sequence remains Unknown until a later valid checkpoint. A received but malformed query never falls back, and Crossplay keeps the fallback disabled because that log path has not been accepted there.

Every Ready Host and Friend card now shows why its timer is not running. The reason distinguishes automatic shutdown being off, an unavailable count, players online, the Host game activity check, and assigned Friend heartbeat/game blockers. An exact zero with all checks safe still shows the shared live deadline and receives the same final in-gate count/activity recheck before graceful Stop.

| Final check | Result |
| --- | --- |
| React/TypeScript and Windows candidate build | Pass: npm reported 0 vulnerabilities; 21 UI modules, three fixtures, and the self-contained Windows x64 EXE built. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| `TogetherServer.ValheimChecks` | Pass: 10 groups, 0 failures. The new query-silent fixture covers initial zero, partial join Unknown, one player, a repeated readiness marker that cannot erase that player, partial leave Unknown, zero, valid/malformed periodic checkpoints, final guarded Stop recheck, timer blocker text, and no fallback for Crossplay. Disposable data: `local-data/valheim-checks/74318ade1c3d49b1818a398d386104d9`. |
| Other focused .NET regressions | Pass: `TogetherServer.Checks` 11 groups, `TogetherServer.MinecraftChecks` 3 groups, and `TogetherServer.MinecraftSetupChecks` 4 groups, all with 0 failures. Disposable data: `local-data/checks/c4fe928b59d34a798795a527d0ad21ef`, `local-data/minecraft-checks/37084088393242439a38a4c7b9d92218`, and `local-data/minecraft-setup-checks/76bbb37cf4844c859fb431e40684eba2`. No game binary was downloaded and no terms were accepted. |
| `TogetherServer.CompanionChecks` on the candidate | Pass: 21 groups, 0 failures. The paired Friend received the Host-owned count/deadline and retained the stale-heartbeat and final zero-only HTTPS Stop protections. Disposable data: `local-data/companion-checks/369d54094cf64678b5fb7302c86f1bcc`. |
| Served and native desktop smokes on the candidate | Pass: 20 served and 19 native desktop groups, 0 failures. The packaged UI includes **Timer not running**, served a 0/10 deadline snapshot, rendered in its native WebView, and completed its fixture lifecycle, native-picker, tray, and startup paths. Disposable data: `local-data/served-smoke/a345c367c1a641fa80daf9f87995c4fb` and `local-data/desktop-smoke/9801116015264bc497a9cf56eb4c0ca5`. |
| Existing real managed Valheim logs | Privacy-safe parser check passed: the latest log was Ready/0, and the latest log containing a completed player lifecycle also ended Ready/0. No IDs or log contents were copied into Git or reported. This confirms compatibility with those saved logs, not a live join/disconnect or save acceptance test. |
| Packaged and installed build | The tested `local-data/release-candidate/TogetherServer.exe` was copied byte-for-byte to `local-data/release/TogetherServer.exe`: file version `0.1.4.0`, 64,200,942 bytes, SHA-256 `87576934A997215EC2289F7FBA076FB6F31B8DA0F43E6646B8D3DEA9B4060956`. The prior installed EXE is preserved as `TogetherServer.exe.pre-private-count-fix`. Nothing was published or pushed. |
| Browser and real-game acceptance | The in-app Browser listed no available browser, so browser screenshot inspection was skipped. The native rendered WebView check passed but does not replace it. No live real Valheim zero/one/disconnect transition, Friend game join, automatic Stop/save/restart, or outside-network test was run. |
...
| Final check | Result |
| --- | --- |
| Release preparation | Pass: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/prepare-github-release.ps1`; npm reported 0 vulnerabilities, 21 UI modules and three fixtures built, and the Windows x64 self-contained single-file publish completed with no errors. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Prepared asset | `local-data/github-release/v0.1.5/TogetherServer-win-x64.exe`, file version `0.1.5.0`, 64,200,945 bytes, SHA-256 `E62BCC43830D8C8130179D424320D58997031EEBEFEDE6C5A6C5360B6441A106`. The adjacent `.sha256` file and `local-data/release-candidate/TogetherServer.exe` matched it. |
| Updater checks | Pass: 6 groups, 0 failures. Covers no-release state, exact asset and digest acceptance, tag-to-EXE version matching, wrong digest, untrusted/prerelease metadata, and verified replacement with prior-EXE preservation. Disposable data: `local-data/update-checks/2cc825ac07e14b779a1f43eb5dc690ba`. |
| Exact release EXE companion checks | Pass: 21 groups, 0 failures, including assigned-server visibility, count/deadline delivery, stale heartbeat handling, zero-only remote Stop, and graceful Ctrl+C. Disposable data: `local-data/companion-checks/d0946a1b35b94473bc079962caf9fce8`. |
...
## 2026-09-22 - Server-count-only timer, Host extension, and faster refresh

Starting Git HEAD: `6bda369`, with the published `v0.1.5` release unchanged. Automatic shutdown no longer stores, requests, or evaluates a Host or Friend game-client path or `gameRunning` report. A Ready server's reliable count is the only occupancy authority: exact zero creates the Host-owned deadline; a positive or unavailable count removes it; and expiry still obtains a fresh count inside the lifecycle gate before the existing graceful Stop path. Friend app connectivity remains visible status but cannot block or authorize the timer.

An active Host card now accepts any positive whole number of extra minutes that fits the supported date range. Added time changes only that active deadline; a positive or unavailable count cancels the extended countdown, and a later zero begins the normal configured wait. Host supervision/player-count polling remains about every three seconds, Friend heartbeat/status polling is now about every five seconds, and both cards continue rendering the shared countdown every second. GitHub release checks now start with the app and repeat every 30 minutes, with the existing manual recheck still available.

| Final check | Result |
| --- | --- |
| Candidate build | Pass: npm reported 0 vulnerabilities; TypeScript/Vite, three fixtures, and the Windows x64 self-contained EXE built. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Host and game lifecycle regressions | Pass: `TogetherServer.Checks` 11 groups, `TogetherServer.ValheimChecks` 10 groups, `TogetherServer.MinecraftChecks` 3 groups, and `TogetherServer.MinecraftSetupChecks` 4 groups, all with 0 failures. The Valheim timer coverage proves extension, player/Unknown cancellation, a fresh normal countdown after cancellation, final zero recheck, and graceful Ctrl+C. Disposable data: `local-data/checks/15683facb21b4b5696a9c5c8ecdc7a90`, `local-data/valheim-checks/fc6254f8955741a18976bb4c2ba15307`, `local-data/minecraft-checks/e8c74e117de94d7eba0103a4b0ee0066`, and `local-data/minecraft-setup-checks/d3a4eeebf4594b77a3c4b234c5e1642c`. |
| Updater regressions and startup check | Pass: 7 groups, 0 failures, including the 30-minute automatic cadence plus existing release/digest/replacement safety. A separate isolated candidate launch completed its automatic startup query and reported installed/latest `0.1.5` as Current without using the manual recheck; data: `local-data/startup-update-check/272640528cb04a2cb6332518b8731718`. Unit data: `local-data/update-checks/b16e934058f745c79a6776716e1e41a1`. |
| Packaged Host/Friend regression | Pass: 21 groups, 0 failures. A paired Friend received the Host's 0/10 count and extended deadline, a positive count removed it, and remote Stop remained zero-only with graceful Ctrl+C. The public heartbeat/device shapes no longer contain game-running state. Disposable data: `local-data/companion-checks/b60e2a35bbd047238851fea9febefdec`. |
| Served and native desktop smokes | Pass: 20 served and 17 final desktop groups, 0 failures. The packaged UI omits the game-activity controls, includes the Host extension control, exercises exact deadline addition, renders in the native WebView at compact size, and completes lifecycle, picker, tray, and startup paths. Disposable data: `local-data/served-smoke/be94ba9f1658411ba5891abe8832f639` and `local-data/desktop-smoke/f625935722c34a6e94898e72a0fcb1b9`. An initial desktop run hit a transient UI Automation lookup immediately after a React preference rerender; the harness now waits briefly for the accessible title-bar control, and the complete rerun passed. |
| Packaged candidate | `local-data/release-candidate/TogetherServer.exe`, file version `0.1.5.0`, 64,197,440 bytes, SHA-256 `88C5EEEF80A02A0D90532C2CC2FB1C6912D3B502A3E14DB8BE5FB5CC7E418C16`. It was not installed, published, or pushed. The owner's running `local-data/release/TogetherServer.exe` and managed server were left untouched. |
| Real-game acceptance | Not run: no live real-game player transition, automatic Stop/save/restart, or outside-network Friend test was performed. Fixture evidence is not a real-game claim. |

No real game binary, world, credential, public listener, router, firewall, DNS setting, existing GitHub release, or running owner process was changed.

## 2026-09-23 - Port-conflict handoff, per-server permissions, and outside reachability

Starting Git HEAD: `7db50a94`, clean `main` worktree. Start now reports a named managed server and its shared protocol/ports before the generic concurrency limit, while the serialized launch gate and Windows bind check remain authoritative. Setup review warns about overlapping saved port plans and suggests the next non-overlapping configured port. A Friend with Start permission for the requested assigned server may explicitly choose **Stop empty server and start this one** without assignment or Stop permission for the conflicting server. Ordinary Start never stops anything. Replacement fails closed unless every conflict is Ready with an exact zero-player count, repeats that count inside the lifecycle gate, preserves an active Host **Add time** extension, preflights the remaining concurrency limit, waits for graceful exit, and only then starts the requested server.

Paired PCs now retain global Start/Stop defaults plus independent permissions for every assigned server. The global checkbox shows an indeterminate dash whenever a server differs from its default, and changing it applies that action to every assigned server. Assignment without Start or Stop still shares connection details. A current successful outside TCP probe is labeled **Reachable outside network** even without a Friend; a fresh authenticated heartbeat takes precedence as **Friend connected** without claiming where that PC is located.

| Final check | Result |
| --- | --- |
| Candidate build | Pass: npm reported 0 vulnerabilities; TypeScript/Vite bundled 22 modules, all three fixtures built, and the Windows x64 self-contained single-file candidate published. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Host and game regressions | Pass: `TogetherServer.Checks` 11 groups, `TogetherServer.ValheimChecks` 10 groups, `TogetherServer.MinecraftChecks` 3 groups, and `TogetherServer.MinecraftSetupChecks` 4 groups, all with 0 failures. Disposable data: `local-data/checks/5d5b0dc7b2b0422d9e2e1fe8ba87fa10`, `local-data/valheim-checks/d4b2e40ded2e4987b9a4cf6e7f4de2c9`, `local-data/minecraft-checks/a9b8d9f9d97f4b509bb66b032a3f4cfd`, and `local-data/minecraft-setup-checks/a808fa9d5c7540d183cf9bb631fafd8b`. No real game download or terms acceptance occurred. |
| Packaged Host/Friend regression | Pass: 22 groups, 0 failures. It covers per-server permission enforcement, scoped global changes that preserve the other action's exceptions, named conflicts, positive/Unknown-count and Host-extension denial, a fresh in-gate zero check, graceful replacement, and a Start-only Friend assigned only to the requested profile. Disposable data: `local-data/companion-checks/9e03acac9bde42678372126ed28ed21c`. |
| Updater regression | Pass: 7 groups, 0 failures. Disposable data: `local-data/update-checks/3fa4654115404db793a5a821979e339a`. |
| Served and native desktop smokes | Pass on the final candidate: 20 served and 17 native desktop groups, 0 failures. The bundled UI contains the duplicate-port warning, explicit replacement action, per-server permission picker, indeterminate global label, and outside-reachability wording; the native WebView rendered and completed its lifecycle, picker, tray, and startup paths. Disposable data: `local-data/served-smoke/de7a89743efa4cae88efff2a78d60e00` and `local-data/desktop-smoke/c4268da2ce3645c7a5714f98afcec525`. |
| Packaged candidate | `local-data/release-candidate/TogetherServer.exe`, file version `0.1.5.0`, 64,210,550 bytes, SHA-256 `404E6A2045009E8A0673BC1CAED526108A113DA811537296D7EDE28FF594C95D`, Authenticode status `NotSigned`. It was not installed, tagged, published, or pushed. |
| External acceptance | Not run: no real-game join/player/save/restart check, manual browser screenshot review, or outside-network Friend PC pairing was performed. Fixture and outside-probe logic are not represented as those tests. |

Exploratory validation exposed two fixture-harness assumptions after the new replacement itself had succeeded: cleanup initially ran before the replacement became Ready, then the second synthetic profile used names the strict fixture rejects. Two later coverage additions also needed test-only sequencing corrections: the global Start test had not restored its old default before an older denial assertion, and the first Unknown-count assertion ran before target authorization. All were corrected before the final 22-group pass. One earlier native smoke reached its final startup assertion after 16 passes and observed a visible startup window; a clean rerun passed, and the rebuilt final candidate then passed all 17 groups directly. No real world, credential, public listener, router, firewall, or DNS setting was used or changed.

## 2026-09-23 - Per-field connection privacy and background desktop smoke

Starting Git HEAD: `75bd17f`, clean `main` worktree. A Ready Host card still keeps Connection details on the main server card rather than under **Technical details**, but now presents **Server IP** and **Game password** as separate masked fields. Each field owns its own eye, 30-second/focus-loss hide behavior, loading state, and Copy button. Copying or showing one field does not expose the other, and the protected Valheim password is fetched only for its own show or copy action. Friend cards use the same per-field control for the server IP; Valheim passwords remain shared separately by the Host.

The native desktop smoke now runs through the existing startup/tray path by default, keeping its WebView window hidden while it checks render, lifecycle, guarded Quit, duplicate launch, mode persistence, and clean exit. `-Interactive` retains the attended visible-window coverage for compact sizing, custom title-bar controls, native pickers, and manual-launch restore behavior.

| Final check | Result |
| --- | --- |
| React/TypeScript and Windows candidate build | Pass: npm reported 0 vulnerabilities; TypeScript/Vite bundled 22 modules; all three fixture projects built; and the Windows x64 self-contained single-file candidate published. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Served packaged smoke | Pass: 20 groups, 0 failures. The exact candidate contains the separate Server IP/Game password controls, protected-password local route, Host/Friend pages, synthetic lifecycle, connection diagnostics, and mode switching. Disposable data: `local-data/served-smoke/ab73cb728ec04dd9bab6d63351539c86`. |
| Background native desktop smoke | Pass: 8 groups, 0 failures. React rendered with custom chrome while hidden in the tray; synthetic Valheim start/stop/restart, guarded Quit, duplicate hidden launch, and Friend-mode persistence all passed. Disposable data: `local-data/desktop-smoke/12326e5d4bfb4117899ce3fb5aa1af61`. |
| Interactive visual acceptance | Not run so the test would not interrupt the owner's active desktop. The in-app Browser reported no available browser instance. The source/bundle checks and hidden native WebView render passed, but they do not replace an attended click-through or screenshot review. |
| Packaged candidate | `local-data/connection-controls-candidate/TogetherServer.exe`, file version `0.1.5.0`, 64,210,640 bytes, SHA-256 `C95D8F33C8C088F492BBE0B281109DBCD2199BB7EC99CDDA2BFD440F87ED935B`, Authenticode status `NotSigned`. The already-running older `local-data/release-candidate/TogetherServer.exe` was left untouched. Nothing was installed, tagged, published, or pushed. |

No real game binary, valued world, credential, public listener, router, firewall, DNS setting, or outside-network route was used or changed. Real-game and public-network acceptance remain separate gates.

## 2026-09-23 - Custom game script manager and game support catalog

Starting Git HEAD: `0037d8520344ba5f5ccd0680977ff0418ea92d11`, clean `main` worktree. Host setup now includes an explicitly advanced **Custom game** path. The owner supplies a game/server name, working and save directory, primary and additional TCP/UDP ports, and three PowerShell actions: a long-running Start wrapper, a bounded Status/players query, and a graceful Stop request. The scripts are protected with the current Windows account rather than written to `host.json`. Removing the profile or changing it away from Custom removes its protected script record.

TogetherServer always starts the fixed Windows PowerShell executable and sends the saved action over standard input. The tracked Start wrapper retains the existing PID, creation-time, and executable-path identity checks. Status has a four-second timeout, 64 KB output limit, and a strict JSON shape for Ready/Starting/Failed, detail, counts, and player names. Stop has a 15-second action timeout and must cause the tracked Start wrapper to exit within 90 seconds. TogetherServer never force-kills the custom game and makes no save-integrity claim.

Friends still submit only an assigned profile ID and typed action. They cannot send or read script text, paths, arguments, or environment values. A permitted Friend may request the profile's fixed Start action, but custom player data is deliberately display-only: it cannot authorize Friend Stop, automatic shutdown, or empty-server replacement. The local Host owns Custom Stop. The new catalog records 113 Windows-first candidate rows plus 17 frequently requested blocked/unverified rows, with a recommended built-in order and primary-source refresh rule. Catalog membership is research/backlog status, not official game support.

| Final check | Result |
| --- | --- |
| Candidate build | Pass: npm reported 0 vulnerabilities; TypeScript/Vite bundled 22 modules; all three fixtures built; and the Windows x64 self-contained single-file EXE published. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Custom lifecycle checks | Pass: 3 groups, 0 failures in the final expanded run. Covers protected/non-plaintext scripts, required and size-bounded actions, the actual Start wrapper PID environment, malformed/oversized/timed-out status, player-list display, exact-wrapper restart reattachment, local graceful Stop, profile-removal cleanup, and denial of Friend Stop, automatic shutdown, and empty-port replacement. The core lifecycle suite also passed five consecutive runs after the Stop race fix. Final data: `local-data/custom-checks/f1f186642ab24b8dac5ed292e6642190`. |
| Existing Host/game regressions | Pass: Host 11 groups, Valheim 10 groups, Minecraft 3 groups, and Minecraft setup 4 groups, all with 0 failures on their final runs. Data: `local-data/checks/35cfac8069264f0fae6c20a81cffb017`, `local-data/valheim-checks/e039a714e9bb45cfa71f1cc1a08af2ba`, `local-data/minecraft-checks/390c530e11764a0cad4ff19ec876946a`, and `local-data/minecraft-setup-checks/1f71f3afd7e34be7a97e2d7182ef33dd`. |
| Packaged Host/Friend regression | Pass: 22 groups, 0 failures against the final candidate, including existing assignment, permission, count, restart, multi-Host, rate-limit, remote Stop, and conflict-replacement gates. Data: `local-data/companion-checks/fd191e4a6c5e414bb29bb134fbabbb66`. |
| Packaged served smoke | Pass: 21 reported groups, 0 failures. It verifies the bundled Custom UI and driver catalog, local mutation/reveal gates, protected storage, packaged Start/status/player list/Stop, display-only safety, cleanup, and all prior synthetic API paths. Data: `local-data/served-smoke/1f15590ff55e465d98c8a6c28f5fefb7`. |
| Background native desktop smoke | Pass: 8 groups, 0 failures, including hidden WebView rendering, managed lifecycle, updater guard, tray, duplicate launch, mode persistence, and Quit. Data: `local-data/desktop-smoke/f596c003b6b242459f5998478fc8d81a`. |
| Updater and handoff regressions | Pass: updater 7 groups and isolated handoff 3 groups, 0 failures. Data: `local-data/update-checks/16247b4c960b4002bbb51772119caf8f` and `local-data/update-handoff/dd06ae9aaa344848ae5b2f9ba1e77e5b`. |
| Packaged candidate | `local-data/release-candidate/TogetherServer.exe`, file version `0.1.5.0`, 64,222,279 bytes, SHA-256 `ED7206EC2DD28FA523ECB14846169C4ED37CAC5E775529C7026BE1B343135571`, Authenticode `NotSigned`. It was not installed, tagged, published, or pushed. |
| Visual and real-game acceptance | The background native WebView rendered, but no attended screenshot/click-through was run. No candidate game's real server, client join, player query, save/restart, or public Friend route was tested. Custom scripts and catalog research are not represented as built-in or real-game acceptance. |

One repeated Custom run exposed a race in `Process.WaitForExitAsync` for an externally attached PowerShell wrapper. Stop now polls the exact verified wrapper until exit and five consecutive Custom suite runs passed. A later Valheim regression run transiently returned process identity `Unknown` before synthetic readiness after seven passing groups; the exact disposable fixture PID/path was verified and cleaned up, and a clean complete rerun passed all 10 groups. No production Valheim logic was changed for that isolated transient.
...
| Final check | Result |
| --- | --- |
| Release preparation | Pass: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/prepare-github-release.ps1`; npm reported 0 vulnerabilities, TypeScript/Vite bundled 23 modules, all three fixtures built, and the Windows x64 self-contained single-file publish completed with no errors. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Exact release asset | `local-data/github-release/v0.1.6/TogetherServer-win-x64.exe`, file version `0.1.6.0`, product version `0.1.6`, 64,222,397 bytes, SHA-256 `E422A0F5F41D6C9B97314BAC62D107936E3B85B6AA72308A7E312EF6BE4B7AC4`, Authenticode `NotSigned`. The adjacent `.sha256` file and `local-data/release-candidate/TogetherServer.exe` matched it exactly. |
| Host and built-in game regressions | Pass: Host lifecycle 11 groups, Valheim 10 groups, Minecraft 3 groups, and Minecraft setup 4 groups, all with 0 failures. Data: `local-data/checks/81f28766696c49c7b093a6c151937b5c`, `local-data/valheim-checks/d46ad32bbcaf4b0898c708082c595672`, `local-data/minecraft-checks/fefdb565f9c243b49cd38f16ccde5633`, and `local-data/minecraft-setup-checks/474cde156aae48e9a570535073d6db76`. |
| Custom and updater regressions | Pass: Custom 3 groups and updater 7 groups, 0 failures. The exact release asset also passed the isolated updater handoff's 3 checks. Data: `local-data/custom-checks/2dba6ca0353944d09d7e065e43064015`, `local-data/update-checks/578611c4def04364bf877904644b53b6`, and `local-data/update-handoff/0139c0f93fbe41babab47f81eff875a1`. |
...
## 2026-09-24 - Friend coordination, bounded pairing, and access lifecycle

Starting Git HEAD: `67f17db`. Reusable open-ended TS3 pairing is replaced by owner-opened windows that default to one new PC for 30 minutes and support a bounded duration, device limit, and optional local approval. Close pairing invalidates future use without revoking paired PCs. Per-device revoke remains separate, while emergency revoke closes the window and invalidates the server-code credential lineage. Existing TS1/TS2 formats remain decodable until their original expiry; an existing pre-window TS3 receives one 30-minute migration window. Copying a current code refreshes only its endpoint and active certificate pin and never silently extends or reopens its window.

Friend mode can rename and forget saved Host connections. Reachable Forget first revokes that device credential over the authenticated pinned channel. Offline or denied Forget removes the protected local credential and explicitly tells the user to ask the Host owner to revoke the stale PC. A multi-use code may leave a newly paired PC in **Awaiting approval** until the Host approves it locally.

Each server has bounded maintenance state and an owner message. Status remains visible, but remote Start, Stop, Restart, replacement, and timer extension are checked and denied both before durable acceptance and again during execution. Local owner lifecycle actions remain available. `CanExtendTimer` is off by default and can be granted globally or per assigned server. A Friend submits only the fixed `extend` action; the Host applies its configured increment and per-countdown maximum. Positive or Unknown occupancy cancels the countdown and resets all Friend extension allowance.

A structured redacted activity feed retains the newest 500 events or 30 days and covers pairing, access changes, remote operations, lifecycle transitions, countdowns, recovery, route changes, backup failures, and expiry warnings. Assigned Friends receive only device-scoped or assigned-server events. Credentials, codes, passwords, Custom scripts, player IPs, paths, and raw arguments are not event fields. Important/Warning events also use the existing native tray. Companion protocol capabilities now advertise pairing windows, maintenance, activity, and timer extension.

| Check | Result |
| --- | --- |
| `scripts/build.ps1` | Pass: npm reported 0 vulnerabilities; TypeScript/Vite bundled 23 modules; all fixtures built; Windows x64 single-file app published. The existing WebView2/WindowsBase `MSB3277` warning remains. |
| Packaged companion checks | Pass: 28 groups, 0 failures. Covers pairing duration/device limits, approval, close, emergency and per-device revoke, rename, reachable/offline Forget, maintenance, filtered activity, fixed timer limits/reset, credential renewal, rate-limit partitions, staged pins, endpoint recovery, durable operations, final zero checks, graceful Stop, and safe replacement. Final-build data: `local-data/companion-checks/dda14e7c3f31461ead955c1a666f9f7e`. |
| Host and game regressions | Pass: core 16, Valheim 10, Minecraft 3, Minecraft setup 4, Custom 5, and updater 7 groups, all with 0 failures. Data: `local-data/checks/1f52b4b4f6aa4ed8a8fffc7eed3cc358`, `local-data/valheim-checks/642cc604397045098367de9e5518f7b4`, `local-data/minecraft-checks/715533d1e7eb466a8e47281a58675302`, `local-data/minecraft-setup-checks/8ce9e35916374709b79d16f21f869ed4`, `local-data/custom-checks/9c021a9f89aa488e974aa32d013bb7df`, and `local-data/update-checks/f1d8825ecdbf47f6a56a86f56d1637de`. |
| Packaged UI and update handoff | Pass: served smoke 21 groups, background native desktop smoke 8 groups, and isolated updater handoff 3 groups, all with 0 failures. Final-build data: `local-data/served-smoke/80afbe723a32491486a75550164fbd97`, `local-data/desktop-smoke/cfb5adfe7901451e9f1096466ec908f3`, and `local-data/update-handoff/de12e58da0c6471c880b43589d056f12`. The desktop smoke proves the hidden native tray/WebView lifecycle; an attended balloon-notification screenshot was not captured. |
| External acceptance and later convenience features | Not run. No real Custom/built-in server, real Friend PC, actual player transition, saved-world restart, private/public route, or valued-world backup/restore was exercised. Start-when-empty, availability windows/quiet hours, permission presets, outbound Discord-style notifications, more built-in drivers, and a signing path remain intentionally gated on the roadmap's real-game acceptance. |

The first expanded companion runs exposed three test/order issues and one actual current-invite transport regression: a connection was forgotten before endpoint recovery reused it, a test resubmitted stale pre-pairing settings, and copied codes no longer refreshed the recovered endpoint/active pin. The harness ordering/settings were corrected, and transport metadata now refreshes without reopening or extending the bounded window. Final audit then found and fixed startup journal pruning being skipped after Interrupted reconciliation, the last snapshot-triggered first probe, and a Windows race when Stop immediately followed Minecraft Start. Regression coverage now enforces newest-500 pruning, cache-only snapshots, and a bounded exact-process console-attach retry. The final package passed the matrix above.

No release tag or GitHub release was created. No real game binary, valued world, real credential, public listener, router, firewall, DNS setting, mesh policy, or installed release was used or changed.

## 2026-09-24 - 0.1.7 architecture, release, recovery, and verification audit

The audit began from Git HEAD `a7de82a`; the evidence below records the final pre-commit verification run. The application and UI development version are aligned at `0.1.7`, and the packaged product version restores the source revision suffix. The repository now pins the .NET SDK and Node release line, commits a reproducible RID-specific NuGet lock graph, defines deterministic build defaults, and has one serial Windows verification path for Git whitespace, UI lint/tests, locked restore, .NET checks, formatting, packaged HTTP smoke, and hidden native desktop smoke. The release-preparation path rejects a dirty worktree, an invalid or non-increasing version, existing tags or output directories, and any final EXE without a valid Authenticode signature. It prepares and hashes an artifact but still does not tag or publish it.

Automatic updates now remain disabled when the installed application is unsigned. A candidate must match its advertised version and digest, have a valid Authenticode signature from the same publisher public key, and remain valid through staging, handoff, and replacement. A failed post-replacement verification restores and re-verifies the prior executable before any relaunch. Signing still requires an owner-supplied certificate and signing tool; no key or secret is stored in the repository.

The Windows x64 single-file package keeps WebView2 Core and WinForms while excluding the unused WPF reference. The exact package's x64 native loader is also bundled at the extraction root because WebView2 environment creation resolves it there. The published directory has no loose WebView runtime DLL dependency. Hidden startup now reports sanitized render/load state and exception class information without exposing local paths or exception messages.

Durable local data records schema version 1. Malformed state is quarantined under `data/quarantine` instead of being overwritten. When authoritative lifecycle state cannot be trusted, `data-recovery.json` keeps lifecycle operations blocked until the owner acknowledges recovery. Minecraft Java discovery accepts only TogetherServer-managed artifacts whose recorded provenance and hash still match; a manifest-shaped or third-party JAR is not treated as managed.

| Check | Result |
| --- | --- |
| Exact serial verifier | Pass: `scripts/verify-release.ps1 -Build` completed from a fresh UI/app build before commit. That candidate `local-data/release-candidate/TogetherServer.exe` was 64,311,835 bytes, file version `0.1.7.0`, product version `0.1.7+a7de82ad0cda9c58383d2c5aaa9b9fca2133d06a`, and SHA-256 `ABD585A8CBC2678C37FB6C9E8D5877F22F22CE75E1D4CE8E5CE60640CE708EA0`. The exact hash remained unchanged through the serial matrix; a post-commit run must produce and report the candidate bound to the implementation commit. |
| UI and repository gates | Pass: npm audited 233 packages with 0 vulnerabilities; production build bundled 31 modules; ESLint passed with zero warnings; Vitest 5 passed 7 files and 23 tests; the Git whitespace gate, locked application restore, and solution formatting verification passed. NuGet reported no vulnerable, deprecated, or outdated packages. TypeScript 6.0.3 is the only intentional npm freshness exception because stable `typescript-eslint` 8.70.1 requires TypeScript below 6.1. |
| Host and game checks | Pass: core 26, Valheim 10, Minecraft 3, Minecraft setup 4, Custom 5, and updater 11 groups, all with 0 failures. Data: `local-data/checks/3052263a5cfb4bbcab1bc76815f5b543`, `local-data/valheim-checks/1ccab028c86e446cb599cf6e4a33ba65`, `local-data/minecraft-checks/7e1b77443d6645eebe27f842e4e3407f`, `local-data/minecraft-setup-checks/73be7c2f4e0a4f97baf7f5e220a6ed9e`, `local-data/custom-checks/340f382dcad343ca8a8895347e156439`, and `local-data/update-checks/b852a3d1b9704428830f5969f17a5904`. |
| Companion and packaged UI checks | Pass: companion 32, served smoke 21, and hidden native desktop smoke 8 groups, all with 0 failures. Data: `local-data/companion-checks/1e35b5485c3b4d7f9d8f9f3ef6a92431`, `local-data/served-smoke/0037b55860ce4e25aa2f40dba04065c7`, and `local-data/desktop-smoke/fee661cdb5c94b6c93732ef97386ee1d`. The in-app browser connection was unavailable, so no attended browser screenshot/click-through was recorded; hidden WebView rendering is not represented as that visual review. |
| Signature-required handoff | Skipped: the development candidate is `NotSigned`. The same-publisher handoff behavior passed injectable updater checks, but a release candidate must be signed with the owner's valid Authenticode certificate and rerun through the signature-required verifier. |
| External acceptance | Not run. No real Valheim or Minecraft server/client, real Friend PC, actual player transition, valued-world save/restart or recovery, private/public route, or router/firewall/DNS change was exercised. Fixture, loopback, package, and native WebView evidence is not represented as those external passes. |

The first commit-bound rebuild exposed a synthetic Custom-check cleanup race: its Start script could delete a shared stop signal that an immediate Stop had already created. HostManager correctly retained the exact process record and did not force-kill it. The fixture now scopes the signal to the managed operation ID and removes it only after observing it; a five-cycle immediate Start/Stop regression plus the full Custom suite passed three consecutive runs (6 groups per run), and the focused Host suite remained 26/26. The final handoff reports the subsequent commit-bound package matrix and artifact identity.

No release tag or GitHub release was created. No real game binary, valued world, real credential, public listener, router, firewall, DNS setting, or installed release was used or changed.

## 2026-09-25 - Isolated production and staging instances

Starting Git HEAD: `804b618`. The same EXE now has an explicit `--staging` instance for candidate testing beside the stable app. Production keeps `%LOCALAPPDATA%\TogetherServer`, loopback UI port 5127, and Friend-control port 5131. Staging uses `%LOCALAPPDATA%\TogetherServer-Staging`, ports 5128 and 5132, a separate process lock and WebView store, an instance marker, independent profiles/settings/runs/credentials, distinct game-port defaults, and visible STAGING window/tray/UI labels. Windows sign-in registration and automatic stable-release updates are unavailable in staging.

Staging refuses a data root equal to, inside, or containing production; refuses a linked or pre-populated unmarked staging root; and production refuses a staging-marked root. Every staged save path must stay in its reviewed staging subroot without crossing a junction. Valheim existing-world browse/import is disabled and only a new disposable world is accepted. Minecraft discovery is limited to staging-managed installs; new Bedrock setup uses 19134/19135 with LAN discovery disabled. Unrestricted Custom scripts are excluded because they could reference production files. Package preparation copies only the candidate EXE, two launchers, and its guide and refuses unexpected output-directory contents.

| Check | Result |
| --- | --- |
| Exact serial verifier | Pass: `scripts/verify-release.ps1 -AppPath local-data/release-candidate/TogetherServer.exe`; file version `0.1.7.0`, pre-commit product identity `0.1.7+804b618fd55a028c04476e52182e76d73d40eb3d`, 64,318,243 bytes, SHA-256 `147CB9005B09F167B0F844DA8C21470788A6EC4A8075AE2C2729D21B0C330C41`, and `NotSigned`. The exact hash remained unchanged through the serial matrix. |
| UI and repository gates | Pass: npm audited 233 packages with 0 vulnerabilities; TypeScript/Vite bundled 31 modules; ESLint passed with zero warnings; Vitest passed 7 files and 24 tests; whitespace, locked restore, solution build, and formatting passed. |
| Host and game checks | Pass: core 28, Valheim 10, Minecraft 3, Minecraft setup 5, Custom 6, and updater 12 groups, all with 0 failures. Data: `local-data/checks/a9d46ff6850c4b4ba204b3fe6c18ef25`, `local-data/valheim-checks/f926b1167023454d843904986753a325`, `local-data/minecraft-checks/2d6f6bd720db4a2295b688d323b69aa0`, `local-data/minecraft-setup-checks/db24f4b17ac54144a1e7d9b378e6cf57`, `local-data/custom-checks/95b7e200e70443a8b3e3e5729bea4a19`, and `local-data/update-checks/37c1d4236f68472e80e1601691f03826`. |
| Production plus staging smoke | Pass: two packaged app processes ran simultaneously with distinct roots and ports; staging began with no profiles/runs, rejected a fake production-world path and import, exposed no existing Valheim worlds, left the fake production save marker and Windows Run value unchanged, kept updates offline, served the visible isolation warning, and closed both processes cleanly. Data: `local-data/staging-smoke/8ad5a48ffaf646289771222c71d3d361`. |
| Companion and packaged UI checks | Pass: companion 32, served smoke 21, and hidden native desktop smoke 8 groups, all with 0 failures. Data: `local-data/companion-checks/d515f52b459e4b5695bf4c16cd01dee9`, `local-data/served-smoke/6abc4460142d4a6fab47f63e90f1b4d0`, and `local-data/desktop-smoke/0e3c1b86b84947e982b88e41b5c7ef14`. |
| Real staging Friend PC and game join | Not run. The package and two-process loopback evidence do not prove TCP 5132 from another network, pinned authentication on a Friend PC, or a real join to the staging game port. Those remain owner-run external acceptance steps with a disposable staging world. |

The prepared `local-data/staging-package` contains exactly `TogetherServer.exe`, the STAGING Host launcher, the STAGING Friend launcher, and `README-STAGING.txt`; no settings, credentials, managed runs, or world files are included. No release tag or GitHub release was created. No real world was copied, overwritten, or deleted, and no router, firewall, DNS, mesh, public listener, or installed stable app setting was changed.

## 2026-09-25 - Unambiguous direct-open development package

Starting Git HEAD: `a6e0aab`. The staging package now contains `TogetherServer DEVELOPMENT.exe`, clearly named DEVELOPMENT Host/Friend launchers, and `README-DEVELOPMENT.txt`; the old ambiguous `TogetherServer.exe` package copy is removed. The development filename itself selects staging, so a direct double-click uses the separate data root and ports instead of handing off to an already-running production app. The native title/tray label say DEVELOPMENT, its tray/taskbar icon is a square `D`, and the in-app warning is a sticky bright-orange **DEVELOPMENT / STAGING** banner. The normal `TogetherServer.exe`, title, round `T` icon, data, and ports retain production behavior.

The pre-commit candidate and development copy were byte-identical at SHA-256 `96D10796F7F6E3D7F5D054761D6D7AFA61255CAB4108C6A84827688041C09860` and were `NotSigned`. The complete serial verifier passed UI lint, 24 UI tests, core 30, Valheim 10, Minecraft 3, Minecraft setup 5, Custom 6, updater 12, companion 34, served smoke 21, simultaneous production/development staging smoke, and hidden native desktop smoke 8. The first full run encountered one transient synthetic Custom certification-start failure; the immediate isolated rerun passed 6/6 and the subsequent complete serial rerun passed cleanly. Signed update handoff remained skipped because the development candidate is unsigned.

No release tag or GitHub release was created. No real game binary, valued world, real Friend PC/network route, or installed production data was used or changed. The simultaneous-process check used isolated synthetic roots and does not prove a real Friend connection or game join.

## 2026-09-25 - Development port contract and cross-talk regression

Starting Git HEAD: `a3f83a4`. A new staging instance correctly returned `null` for its not-yet-configured Friend endpoint, but the React port-diagnostics decoder required text and replaced the app with `InvalidResponse`. The decoder and its TypeScript view now accept the API's nullable endpoint contract. The DEVELOPMENT / STAGING banner also displays the actual local-app and Friend-control ports so the owner can immediately distinguish the two instances.

The default production and development port plans are asserted to have no numeric overlap: production uses local 5127, Friend control 5131, Valheim 2456-2457, Java 25565, and Bedrock 19132-19133; development uses local 5128, Friend control 5132, Valheim 2458-2459, Java 25566, and Bedrock 19134-19135. The simultaneous-process smoke now verifies distinct API ports and data roots, production/development control-port diagnostics, and a valid `null` unconfigured endpoint from both isolated APIs.
