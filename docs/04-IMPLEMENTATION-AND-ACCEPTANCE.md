...
- Test the everyday GUI path: new or copied world, Save and start, one reusable code per saved server with no separate IP entry, immediate listener enable/disable, scoped refresh revocation, and action-first Host/Friend cards. TS1 and TS2 per-device invites remain readable until expiry.
