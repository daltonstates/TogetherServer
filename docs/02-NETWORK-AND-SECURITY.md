...
- The Friend screen accepts a plain Host IPv4 address with TCP port 5131 assumed; `IP:port` is available when the Host chose another port. First pairing also requires the one-time code that carries the pinned identity and per-device authorization. The entered address must match that code. Host mode prepares its detected app address when the owner creates the first code, while leaving the public listener disabled until the owner enables it.
