...
- The Friend screen needs one pasted code for current pairing. The Host's **Create code and allow connections** action prepares the detected app address and enables the HTTPS listener in the same process. A `TS3` code remains current until the owner refreshes it and may pair several Friend PCs. Refresh invalidates the previous code and every device credential issued through it for that server. Codes and credentials belonging to other saved servers remain valid. Older `TS1` and `TS2` per-device invites remain accepted until their original expiry; `TS1` still needs a separate Host IP. No Friend IP is entered.
