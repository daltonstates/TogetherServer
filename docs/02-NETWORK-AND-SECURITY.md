...
- The Friend screen needs one pasted code for current pairing. The Host's **Invite friend** action prepares the detected app address, enables the HTTPS listener in the same process, and copies the current code. A `TS3` code remains current until the owner chooses **Refresh access** and may pair several Friend PCs. Refresh invalidates the previous code and every device credential issued through it for that server. Codes and credentials belonging to other saved servers remain valid. Older `TS1` and `TS2` per-device invites remain accepted until their original expiry; `TS1` still needs a separate Host IP. No Friend IP is entered.
- Each Friend **device** gets a distinct random credential with expiration and owner-set Start/Stop permissions. Host stores device credential hashes. The reusable server pairing secret is stored only with Windows data protection so the owner can copy the same current code again; Friend stores its own credential through the same protected storage. Do not put codes or credentials in URLs, logs, screenshots, or Git.
